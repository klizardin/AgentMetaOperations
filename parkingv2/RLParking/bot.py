...
from env.vehicle import Vehicle, StaticEnv, VEHICLE_POSSIBLE_OPERATIONS
...
class AsyncVehicleBot:
    ...
    def _draw_net_results(self, ops, ops_values, coefs):
        ...
        for i, possiblie_ops in enumerate(VEHICLE_POSSIBLE_OPERATIONS):
            ...
            x1 = between(settings.NET_SHOW_RECT.left, settings.NET_SHOW_RECT.right, i / (net_results_size + 1))
            x2 = between(settings.NET_SHOW_RECT.left, settings.NET_SHOW_RECT.right, (i+1) / (net_results_size + 1))
            y1 = between(settings.NET_SHOW_RECT.top, settings.NET_SHOW_RECT.bottom, v*0.8 + 0.5)
            y2 = between(settings.NET_SHOW_RECT.top, settings.NET_SHOW_RECT.bottom, 0.5)
            y1, y2 = min((y1, y2)), max((y1, y2))
            rc = Rect(top_left=Point(x1, y1), bottom_right=Point(x2, y2))
            if math.fabs(v) >= 0.5:
                clr = (128, 0, 0)
            else:
                clr = (128, 128, 128)
            self._draw_rect(rc, coefs, clr)
        x1 = between(settings.NET_SHOW_RECT.left, settings.NET_SHOW_RECT.right,
                     net_results_size / (net_results_size + 1))
        x2 = between(settings.NET_SHOW_RECT.left, settings.NET_SHOW_RECT.right,
                     (net_results_size + 1) / (net_results_size + 1))
        rc = Rect(top_left=Point(x1, settings.NET_SHOW_RECT.top), bottom_right=Point(x2, settings.NET_SHOW_RECT.bottom))
        self._draw_rect(rc, coefs)
