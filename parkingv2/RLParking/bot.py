...
class AsyncVehicleBot:
    ...
    def _draw_net_results(self, ops, ops_values, coefs):
        ...
        x1 = between(settings.NET_SHOW_RECT.left, settings.NET_SHOW_RECT.right, net_results_size / (net_results_size + 1))
        x2 = between(settings.NET_SHOW_RECT.left, settings.NET_SHOW_RECT.right, (net_results_size + 1) / (net_results_size + 1))
        rc = Rect(
            top_left=Point(x1, settings.NET_SHOW_RECT.top)
            , bottom_right=Point(x2, settings.NET_SHOW_RECT.bottom)
        )
        self._draw_rect(rc, coefs)
