...
class VehicleTestCase(unittest.TestCase):
    def test_operations(self):
        ...
        self.assertEqual(v.get_rotate_count(), VEHICLE.ROTATE_ACCELERATION.shape[0] // 2)
    ...
    def test_step_velocity(self):
        ...
        v.step(long_time, [])
        self.assertAlmostEqual(v.velocity, VEHICLE.TRANSMISSION_SPEED[-1], 1e-2)
        #v.commands([VehicleOp.NONE, ])
        ...
        v.command_backward(v.get_backward_count())
        v.step(long_time, [])
        self.assertAlmostEqual(v.velocity, VEHICLE.TRANSMISSION_SPEED[0], 1e-2)
        #v.commands([VehicleOp.NONE, ])
        #v.command_backward(v.get_backward_count())
        #v.step(long_time, [])
        #self.assertAlmostEqual(v.velocity, 0.0, 1e-2)
    ...
    def test_step_turn(self):
        ...
        v.step(long_time, [])
        self.assertAlmostEqual(v.wheel_angle, VEHICLE.ROTATE_ANGLE_BOUNDS[0], 1e-2)
        v.commands([VehicleOp.NONE, ])
        v.command_turn(v.get_rotate_count())
        v.step(long_time, [])
        self.assertAlmostEqual(v.wheel_angle, VEHICLE.ROTATE_ANGLE_BOUNDS[-1], 1e-2)
