...
from env.vehicle import Vehicle, VehicleState, StaticEnv
...
class AsyncTrain:
    ...
    def process(self):
        try:
            db_size = 0
            while len(self._db) < GAME_STATES_IN_DB_TO_START_TRAIN:
                _ = yield Request(RequestType.NOP)
                if len(self._db) >= db_size + GAME_STATES_IN_DB_STEP_SIZE:
                    self._db.print_size()
                    db_size = (len(self._db) // GAME_STATES_IN_DB_STEP_SIZE) * GAME_STATES_IN_DB_STEP_SIZE
            ...
            while True:
                ...
                if len(self._db) > GAME_STATES_IN_DB_SIZE_MAX2:
                    self._db.reduce_size()
                    self._db.save(PATHES.get_train_state())
    ...
    def _prepare_values(self, states: List[TrainState], vehicle: Vehicle):
        ...
        for i,train_state in enumerate(states):
            if train_state.final_state:
                ...
                values[i] = (
                    request.get_best_operation_value()*VALUES_TO_REWARD_COEF*RL_RO_COEF
                    + train_state.reward
                    )
        return np.clip(values * REWARD_TO_VALUES_COEF, np.float32(-0.5), np.float32(0.5))
    ...
    def _prepare_inputs_values2(self, states: List[TrainState], vehicle: Vehicle):
        ...
        if values:
            ...
            values = np.clip(values * REWARD_TO_VALUES_COEF, np.float32(-0.5), np.float32(0.5))
        return inputs, ops, values

    def _process_main(self, vehicle: Vehicle):
        states = self._db.get_items(count=STATES_TO_TRAIN)
